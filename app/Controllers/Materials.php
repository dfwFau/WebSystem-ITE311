        ...
        if ($file && $file->isValid() && !$file->hasMoved()) {
            ...
            foreach ($enrolledStudents as $student) {
                $notificationModel->insert([
                    ...
                    'message' => "New material <b>{$filename}</b> has been uploaded by your teacher in <b>{$course['course_number']}</b>.",
                    'is_read' => 0,
                    'created_at' => date('Y-m-d H:i:s'),
                ]);
            }
            ...
            $notificationModel->insert([
                'user_id' => $userId,
                'message' => "You successfully uploaded <b>{$filename}</b> to <b>{$course['course_number']}</b>.",
                'is_read' => 0,
                'created_at' => date('Y-m-d H:i:s'),
            ]);
